...
export function registerCommentTools(server: McpServer, client: CodecksClient): void {
  ...
  server.registerTool("close_comment", {
    title: "Close Comment",
  ...
  server.registerTool("reopen_comment", {
  ...
  server.registerTool("list_conversations", {
    ...
    try {
      ...
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(sanitizeConversations(result))) }] };
    } catch (err) {
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(handleError(err))) }] };
    }
  });
