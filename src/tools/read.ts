...
export function registerReadTools(server: McpServer, client: CodecksClient): void {
  ...
  server.registerTool("list_cards", {
    ...
    inputSchema: z.object({
      ...
      status: z.string().optional().describe("Comma-separated: not_started, started, done, blocked, in_review"),
      ...
      sort: z.enum(["status", "priority", "effort", "deck", "title", "owner", "updated", "created"]).optional(),
    ...
    try {
      ...
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(payload)) }] };
    } catch (err) {
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(handleError(err))) }] };
    }
  });
  ...
  server.registerTool("get_card", {
    title: "Get Card",
    description: "Get full card details (content, checklist, sub-cards, conversations, hand status).",
    inputSchema: z.object({
      card_id: z.string().describe("Full 36-char UUID"),
      include_content: z.boolean().default(true).describe("False to strip body for metadata-only checks"),
    ...
    try {
      ...
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(sanitizeCard(result))) }] };
    } catch (err) {
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(handleError(err))) }] };
    }
  });
  ...
  server.registerTool("list_activity", {
    ...
    try {
      ...
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(sanitizeActivity(result))) }] };
    } catch (err) {
      return { content: [{ type: "text", text: JSON.stringify(finalizeToolResult(handleError(err))) }] };
    }
  });

  server.registerTool("pm_focus", {
    title: "PM Focus",
